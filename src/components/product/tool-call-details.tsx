  ...
  return (
    <div className="rounded-md border border-line bg-raised">
      ...
      >
        ...
        <span className={STATUS_TONE[call.status] ?? "text-muted"}>{STATUS_LABEL[call.status] ?? call.status}</span>
        {call.isDemo && <span className="rounded-sm border border-dashed border-line-strong px-1 text-muted">demo connection</span>}
        {call.dryRun && <span className="text-muted">dry run</span>}
      ...
      {open && (
        <div className="space-y-2 border-t border-line px-2.5 py-2 text-2xs">
          ...
          {call.output !== null && call.output !== undefined && <Payload label="Output" value={call.output} />}
        </div>
      )}
    </div>
  );
