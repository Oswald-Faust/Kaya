...
export function SiteNav({ appHref = null, demoHref = "/demo" }: { appHref?: string | null; demoHref?: string }) {
  ...
  return (
    <header className="sticky top-0 z-50 h-[76px] px-3 pt-3">
      ...
      >
        <nav aria-label="Main" className="flex h-14 items-center gap-1 pr-2 pl-4">
          <Link href="/" aria-label="Kaya home" onClick={close} className="mr-5">
            <KayaWordmark />
          </Link>
            ...
            {NAV.map((m) => (
              <details key={m.id} className="group border-b border-line">
                ...
                <div className="space-y-4 px-2 pb-4">
                  {m.columns.map((col) => (
                    <div key={col.title}>
                      <p className="font-mono text-[11px] tracking-wide text-subtle uppercase">{col.title}</p>
    ...
    <>
      ...
      <span className="min-w-0">
        <span className="flex items-center gap-2 text-[15px] text-ink">
          {item.label}
          {item.soon && <span className="rounded-full bg-sunken px-1.5 py-0.5 font-mono text-[9px] tracking-wide text-subtle uppercase">Soon</span>}
        </span>
        {item.desc && <span className="block text-[13px] leading-snug text-muted">{item.desc}</span>}
      </span>
    </>
  );
    ...
    <div className="grid gap-6 border-t border-line p-5 lg:grid-cols-[1fr_320px]">
      <div className={cn("grid gap-6", COLS[menu.columns.length])}>
        {menu.columns.map((col, i) => (
          <motion.div key={col.title} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease: EASE, delay: 0.04 * i }}>
            <p className="px-1.5 font-mono text-[11px] tracking-[0.12em] text-subtle uppercase">{col.title}</p>
      ...
      >
        ...
        <p className="relative font-mono text-[11px] tracking-[0.12em] text-ink/60 uppercase">{menu.featured.kicker}</p>
        <p className="relative mt-1 max-w-[16rem] text-lg leading-snug font-medium tracking-[-0.02em] text-ink">{menu.featured.title}</p>
