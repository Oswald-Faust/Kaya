...
const KIND_LABEL: Record<string, string> = {
  verified: "Found on your site",
  inferred: "Inferred",
  hypothesis: "Hypothesis",
  user_correction: "From you",
  learning: "Learned from an experiment",
};

const SECTIONS: { id: string; title: string; categories: string[] }[] = [
  { id: "identity", title: "What it is", categories: ["identity", "positioning"] },
  { id: "pricing", title: "Pricing", categories: ["pricing"] },
  { id: "features", title: "Core features", categories: ["feature"] },
  { id: "brand", title: "Voice, channels and proof", categories: ["brand", "channel", "traction", "market"] },
  { id: "context", title: "Context you added", categories: ["context"] },
];
...
export function ReviewBoard({ slug, facts, icps, competitors, warnings }: { slug: string; facts: FactView[]; icps: IcpView[]; competitors: CompetitorView[]; warnings: string[] }) {
  ...
  return (
    <div className="mt-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-line bg-surface px-4 py-3">
        <div className="min-w-[200px] flex-1">
          <p className="text-sm text-ink tabular">
            <span className="font-semibold">{reviewed}</span> of {reviewable} confirmed
          </p>
        ...
        {highConfidence.length > 0 && (
          <Button size="sm" variant="secondary" pending={bulk.pending} icon={<Check className="size-3.5" />} onClick={() => bulk.run(() => confirmFactsAction(slug, highConfidence.map((f) => f.id)))}>
            Confirm {highConfidence.length} found verbatim on your site
          </Button>
        )}
      ...
      <FactSection slug={slug} title="What it is" facts={facts.filter((f) => SECTIONS[0].categories.includes(f.category))} />
      ...
      <section aria-labelledby="audience-title">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {icps.length === 0 && <p className="text-sm text-muted">No audience was found. Add context below describing who pays for {"the product"}.</p>}
        </div>
      </section>

      {SECTIONS.slice(1, 3).map((s) => (
        <FactSection key={s.id} slug={slug} title={s.title} facts={facts.filter((f) => s.categories.includes(f.category))} />
      ))}

      <CompetitorSection slug={slug} competitors={competitors} />

      {SECTIONS.slice(3).map((s) => (
        <FactSection key={s.id} slug={slug} title={s.title} facts={facts.filter((f) => s.categories.includes(f.category))} hideWhenEmpty />
      ))}
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-line bg-surface/95 backdrop-blur-sm">
        <form action={finishReviewAction.bind(null, slug)} className="mx-auto flex max-w-[920px] flex-wrap items-center justify-between gap-3 px-5 py-3">
          <p className="text-sm text-muted">
            {remaining > 0 ? `${remaining} unreviewed item${remaining === 1 ? "" : "s"} will stay out of decisions until you confirm them.` : "Everything is reviewed. Nice."}
          </p>
          <button type="submit" className="inline-flex h-9 items-center gap-2 rounded-md bg-ink px-4 text-sm font-medium text-white hover:bg-ink-hover">
            Continue to goal <ArrowRight className="size-4" />
          </button>
        </form>
      </div>
    </div>
  ...
  return (
    <section aria-label={title}>
      ...
      <ul className="divide-y divide-line overflow-hidden rounded-lg border border-line bg-surface">
        ...
        {facts.length === 0 && <li className="px-4 py-3 text-sm text-subtle">Nothing found. Add it as context below if it matters.</li>}
      </ul>
    </section>
  );
}
        ...
        <div className="flex items-start gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-2xs text-muted">
              <span className={cn(fact.kind === "user_correction" && "text-agent")}>{KIND_LABEL[fact.kind] ?? fact.kind}</span>
          ...
          <div className="flex shrink-0 items-center gap-1">
            {confirmed ? (
              ...
              <IconButton label="Confirm" onClick={() => m.run(() => confirmFactsAction(slug, [fact.id]))} disabled={m.pending} tone="positive">
                <Check />
              </IconButton>
            )}
            <IconButton label="Edit" onClick={() => setEditing(true)} disabled={m.pending}>
              <Pencil />
            </IconButton>
            <IconButton label="Remove" onClick={() => m.run(() => rejectFactAction(slug, fact.id))} disabled={m.pending}>
              <X />
            </IconButton>
          </div>
        </div>
...
function IcpCard({ slug, icp, primary }: { slug: string; icp: IcpView; primary: boolean }) {
  ...
  return (
    <article className={cn("rounded-lg border bg-surface p-4", primary ? "border-line-strong md:col-span-2" : "border-line")}>
      <p className={cn("text-2xs font-medium", primary ? "text-agent" : "text-subtle")}>{primary ? "We think your primary audience is" : "Also possible"}</p>
      {editing ? (
  ...
  return (
    <section aria-labelledby="competitors-title">
      <h2 id="competitors-title" className="mb-2 text-sm font-semibold text-ink">
        Competitors buyers compare you with
      </h2>
      <ul className="divide-y divide-line overflow-hidden rounded-lg border border-line bg-surface">
        {competitors.map((c) => (
        ...
        <li className="px-4 py-2.5">
          ...
          >
            ...
            <input id="new-competitor" value={name} onChange={(e) => setName(e.target.value)} placeholder="Add a competitor" className="h-8 min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-subtle" />
  ...
  return (
    <li className={cn("flex items-center gap-3 px-4 py-2.5", confirmed && "bg-raised/60")}>
      ...
      {confirmed ? (
        ...
        <IconButton label={`Confirm ${c.name}`} onClick={() => m.run(() => reviewCompetitorAction(slug, c.id, "confirmed"))} disabled={m.pending} tone="positive">
          <Check />
        </IconButton>
      )}
      <IconButton label={`Remove ${c.name}`} onClick={() => m.run(() => reviewCompetitorAction(slug, c.id, "rejected"))} disabled={m.pending}>
        <X />
      </IconButton>
    </li>
  );
  ...
  return (
    <section aria-labelledby="context-title" className="rounded-lg border border-dashed border-line-strong bg-surface p-4">
      <h2 id="context-title" className="text-sm font-semibold text-ink">
        Anything the site doesn&apos;t say?
      </h2>
      <p className="mt-0.5 text-xs text-muted">Current traction, who actually pays, what you&apos;ve tried before, channels to avoid. It&apos;s saved as confirmed context.</p>
      ...
      <textarea
        ...
        placeholder="e.g. Most paying customers come from word of mouth. We tried LinkedIn ads last year and it didn't work."
