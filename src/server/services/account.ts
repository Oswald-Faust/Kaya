...
import { auth as AUTH_COPY } from "@/i18n/dictionaries/en/auth";

const AUTH_MESSAGES = AUTH_COPY.errors;

const Email = z.string().trim().toLowerCase().email("invalidEmail").max(254, "invalidEmail");
const Password = z.string().min(8, "passwordShort").max(200, "passwordLong");
const Name = z.string().trim().min(1, "nameRequired").max(80, "nameRequired");

function parse<T>(schema: z.ZodType<T>, value: unknown): T {
  const result = schema.safeParse(value);
  if (!result.success) {
    const key = result.error.issues[0]?.message ?? "invalidEmail";
    throw new DomainError("validation", AUTH_MESSAGES[key as keyof typeof AUTH_MESSAGES] ?? "Check the form and try again.", { authKey: key });
  }
  return result.data;
}
...
export function checkRateLimit(key: string, max = 10, windowMs = 15 * 60_000): void {
  ...
  entry.count += 1;
  if (entry.count > max) throw new DomainError("validation", "Too many attempts. Wait a few minutes and try again.", { i18n: "tooManyAttempts" });
}
...
export async function signUpWithPassword(input: { name: unknown; email: unknown; password: unknown }, guestId: string | null): Promise<string> {
  ...
  if (existing && !existing.isGuest) throw new DomainError("conflict", AUTH_MESSAGES.emailTaken, { authKey: "emailTaken" });
...
export async function logInWithPassword(input: { email: unknown; password: unknown }): Promise<string> {
  ...
  if (!user || user.isGuest || !user.passwordHash || !ok) throw new DomainError("validation", AUTH_MESSAGES.wrongCredentials, { authKey: "wrongCredentials" });
  if (user.suspendedAt) throw new DomainError("forbidden", AUTH_MESSAGES.suspended, { authKey: "suspended" });
  return user.id;
}
