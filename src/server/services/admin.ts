...
export async function createUser(admin: AdminContext, input: CreateUserInput) {
  const email = input.email.trim().toLowerCase();
  const name = input.name.trim();

  if (!email || !name) {
    throw new DomainError("validation", "Name and email are required.");
  }
  ...
  const rawPassword = isAutoGenerated
    ? `Kaya-${randomBytes(3).toString("hex")}-${randomBytes(3).toString("hex")}`
    : input.password!.trim();

  if (rawPassword.length < 8) {
    throw new DomainError("validation", "Password must be at least 8 characters long.");
  }

  const passwordHash = await hashPassword(rawPassword);
  const userId = newId("usr");
  const plan = input.plan && ["none", "free", "launch", "growth", "scale"].includes(input.plan) ? input.plan : "launch";
  const planStatus = plan === "none" ? "none" : plan === "free" ? "active" : "trialing";
  const trialEndsAt = planStatus === "trialing" ? new Date(Date.now() + 14 * DAY) : null;

  const orgId = newId("org");
  const workspaceId = newId("ws");
  const baseSlug = (input.workspaceName || name)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
  ...
  await db.transaction(async (tx) => {
    // If upgrading an existing guest with same email
    if (existing?.isGuest) {
    ...
    const effectiveUserId = existing?.isGuest ? existing.id : userId;
    ...
    await adminAudit(admin, "user.created", "user", effectiveUserId, {
      email,
      name,
      plan,
      isPlatformAdmin: Boolean(input.isPlatformAdmin),
  ...
  return {
    user: {
      id: existing?.isGuest ? existing.id : userId,
      name,
      email,
      plan,
    },
...
export async function getImpersonateTarget(admin: AdminContext, targetUserId: string) {
  const target = await db.query.users.findFirst({ where: eq(t.users.id, targetUserId) });
  if (!target) throw new DomainError("not_found", "Target user not found.");
  if (target.suspendedAt) throw new DomainError("forbidden", "Cannot impersonate a suspended user.");

  // Find their first workspace
  const [firstWs] = await db
    .select({ slug: t.workspaces.slug })
    .from(t.workspaces)
    .innerJoin(t.members, eq(t.members.organizationId, t.workspaces.organizationId))
    .where(eq(t.members.userId, targetUserId))
    .orderBy(desc(t.workspaces.createdAt))
    .limit(1);

  await adminAudit(admin, "user.impersonated", "user", targetUserId, {
    targetEmail: target.email,
    targetName: target.name,
  });

  return firstWs ? `/w/${firstWs.slug}` : "/start";
}
...
export async function updateUser(
  ...
  const updates: Partial<typeof t.users.$inferInsert> = {};
  if (input.name && input.name.trim()) updates.name = input.name.trim();
  if (input.email && input.email.trim()) updates.email = input.email.trim().toLowerCase();
  if (typeof input.isPlatformAdmin === "boolean") {
    if (userId === admin.userId && !input.isPlatformAdmin) {
      throw new DomainError("validation", "You cannot remove your own admin access.");
    }
    updates.isPlatformAdmin = input.isPlatformAdmin;
  }
  if (typeof input.suspended === "boolean") {
    if (userId === admin.userId && input.suspended) {
      throw new DomainError("validation", "You cannot suspend your own account.");
    }
    updates.suspendedAt = input.suspended ? new Date() : null;
  }

  if (Object.keys(updates).length > 0) {
    await db.update(t.users).set(updates).where(eq(t.users.id, userId));
    await adminAudit(admin, "user.updated", "user", userId, { updates });
  }
}
...
export async function getWorkspaceDetail(workspaceId: string) {
  ...
  if (!ws) return null;
  const [org, products, members, runs, activity, [{ experiments }], [{ facts }], factsList, experimentsList, goal, integrationsList] = await Promise.all([
    ...
    db.select().from(t.businessFacts).where(eq(t.businessFacts.workspaceId, ws.id)).orderBy(desc(t.businessFacts.confidence)).limit(50),
    db.select().from(t.experiments).where(eq(t.experiments.workspaceId, ws.id)).orderBy(desc(t.experiments.number)).limit(50),
    db.query.goals.findFirst({ where: eq(t.goals.workspaceId, ws.id), orderBy: desc(t.goals.createdAt) }),
    db.select().from(t.integrations).where(eq(t.integrations.workspaceId, ws.id)),
  ]);
  return { ws, org: org!, products, members, runs, activity, experiments, facts, factsList, experimentsList, goal, integrationsList };
}
...
export async function updateWorkspace(
  ...
  const updates: Partial<typeof t.workspaces.$inferInsert> = {};
  if (input.name && input.name.trim()) updates.name = input.name.trim();
  if (input.autonomyMode) updates.autonomyMode = input.autonomyMode;

  if (Object.keys(updates).length > 0) {
    await db.update(t.workspaces).set(updates).where(eq(t.workspaces.id, workspaceId));
    await adminAudit(admin, "workspace.updated", "workspace", workspaceId, { updates });
  }
}
