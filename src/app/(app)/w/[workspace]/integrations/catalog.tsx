...
import { INTEGRATIONS, type IntegrationDomain } from "@/server/integrations/catalog";
        ...
        return (
          <section key={domain} aria-labelledby={`int-${domain}`}>
            <h2 id={`int-${domain}`} className="mb-2 text-sm font-semibold text-ink">
              {t.integrations.domains[domain] ?? domain}
            </h2>
            <div className="grid items-start gap-2.5 md:grid-cols-2">
              {items.map((def) => (
                <IntegrationCard
                  ...
                  recommendation={recommendationFor(def, monthlyBudget, t.onboarding.connect.recommendations)}
                  canManage={canManage}
                  isDemo={isDemo}
